...
interface ApprovalConfig {
    ...
    triggers: {
        newRequest: string[]; // e.g., ["#approve", "اعتماد"]
        approve: string[];    // ["approve", "✅"]
        reject: string[];     // ["reject", "❌"]
        changes: string[];    // ["changes", "تعديل"]
    };
    rules: {
...
const DEFAULT_CONFIG: ApprovalConfig = {
    ...
    triggers: {
        newRequest: ["#approve", "اعتماد"],
        approve: ["approve", "✅", "yes"],
        reject: ["reject", "❌", "no"],
        changes: ["changes", "edit", "تعديل"],
    },
...
module.exports = class WhatsAppApprovalWorkflow {
    ...
    async execute(context) {
        ...
        if (this.isNewRequest(text)) {
            if (!this.isDesigner(sender)) return context.reply("⛔ Not authorized to submit designs.");
            if (this.config.rules.requireMedia && !hasMedia) return context.reply("⚠️ Media required for approval request.");
            ...
            if (this.config.groups.gm) {
                await context.send(this.config.groups.gm, \`🆕 *New Design Request* \\n🆔: \${reqId}\\n👤: \${sender}\\n\\nReply with 'approve' or 'reject'\`);
                if (hasMedia) await context.forward(this.config.groups.gm); 
            }
            
            return context.reply(\`✅ Request \${reqId} submitted for GM approval.\`);
        }
        ...
        if (this.isReject(text)) {
             return context.reply("❌ Rejected (Simulation)");
        }
    }
